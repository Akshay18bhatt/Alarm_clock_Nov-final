...
form.addEventListener("submit" ,(e)=>{
    ...
    let hh=Number(form.hours.value), mm=Number(form.minutes.value), ss=Number(form.seconds.value);
    if(hh==0 && mm==0 && ss==0){
...
function start_timer(list_item_div,total_time_till_zero){
    ...
    const timeOver= setInterval(()=>{
        ...
        if(total_time_till_zero==0){
            ...
            hrs_div.innerText= Math.floor(total_time_till_zero/3600)<10?"0"+Math.floor(total_time_till_zero/3600):Math.floor(total_time_till_zero/3600);
            min_div.innerText= Math.floor((total_time_till_zero%3600)/60)<10?"0"+Math.floor((total_time_till_zero%3600)/60):Math.floor((total_time_till_zero%3600)/60);
            sec_div.innerText= total_time_till_zero%60<10?"0"+total_time_till_zero%60:total_time_till_zero%60;
        }
